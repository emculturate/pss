...
public class SqlParseEventWalker extends SQLSelectParserBaseListener {
	...
	*/
	...
	public void exitCreate_function_expression( SQLSelectParserParser.Create_function_expressionContext ctx) {
		...
		// Walked children: name, optional parameters (String), data_type (Map), clauses (String).
		List<Object> children = extractOrderedRuleChildren(subMap);
		Map<String, Object> createNode = new LinkedHashMap<String, Object>();
		...
		if (index < children.size()) {
			createNode.put(MUMBLE_NAME_KEY, children.get(index++));
		}
		if (index < children.size() && children.get(index) instanceof String) {
			createNode.put(MUMBLE_PARAMETERS_KEY, children.get(index++));
		}
		if (index < children.size()) {
			createNode.put(MUMBLE_DATATYPE_KEY, children.get(index++));
		}
		if (index < children.size() && children.get(index) instanceof String) {
			createNode.put(MUMBLE_CLAUSES_KEY, children.get(index++));
		}
	...
	public void exitCreate_procedure_expression( SQLSelectParserParser.Create_procedure_expressionContext ctx) {
		...
		// Walked children: name, optional parameters (String), required clauses (String).
		List<Object> children = extractOrderedRuleChildren(subMap);
		Map<String, Object> createNode = new LinkedHashMap<String, Object>();
	...
	public void exitCreate_macro_expression( SQLSelectParserParser.Create_macro_expressionContext ctx) {
		...
		// Walked children: name, optional parameters (String); query under MUMBLE_QUERY_KEY.
		List<Object> children = extractOrderedRuleChildren(subMap);
		...
		if (children.size() >= 2 && children.get(1) instanceof String) {
			createNode.put(MUMBLE_PARAMETERS_KEY, children.get(1));
		}
		if (queryChild != null) {
...
*/
	...
	@SuppressWarnings("unchecked")
	private static void attachExtractSource(Map<String, Object> extractItem, Object sourceObj) {
		if (!(sourceObj instanceof Map)) {
			extractItem.put(MUMBLE_EXTRACT_SOURCE_KEY, sourceObj);
			return;
		}
		Map<String, Object> sourceMap = (Map<String, Object>) sourceObj;
		Object sourceType = sourceMap.remove(MUMBLE_EXTRACT_SOURCE_TYPE_KEY);
		extractItem.put(MUMBLE_EXTRACT_SOURCE_KEY, sourceMap);
		if (sourceType != null) {
			extractItem.put(MUMBLE_EXTRACT_SOURCE_TYPE_KEY, sourceType);
		}
	}

	private static void populateTypedDatetimeLiteralSourceMap(Map<String, Object> subMap, String sourceType,
			String characterStringLiteralText) {
		subMap.clear();
		if (characterStringLiteralText != null) {
			subMap.put(MUMBLE_LITERAL_KEY, stripSqlCharacterStringLiteral(characterStringLiteralText));
		}
		subMap.put(MUMBLE_EXTRACT_SOURCE_TYPE_KEY, sourceType);
	}

	private static String stripSqlCharacterStringLiteral(String text) {
		if (text == null) {
			return null;
		}
	...
	public void exitUnsigned_literal( SQLSelectParserParser.Unsigned_literalContext ctx) {
		...
		if (keys.length == 1) {
			...
			if (item instanceof Map<?, ?> typed
					&& ((Map<?, ?>) typed).containsKey(MUMBLE_EXTRACT_SOURCE_TYPE_KEY)) {
				subMap.putAll((Map<String, Object>) item);
			} else {
	...
	@Override
	public void exitDate_literal(SQLSelectParserParser.Date_literalContext ctx) {
		int ruleIndex = ctx.getRuleIndex();
		Integer stackLevel = walker.currentStackLevel(ruleIndex);
		Map<String, Object> subMap = walker.getNodeMap(ruleIndex, stackLevel);
		subMap.remove(ASTWALKER_RULE_TYPE_KEY);
	...
	@Override
	public void exitTime_literal(SQLSelectParserParser.Time_literalContext ctx) {
		int ruleIndex = ctx.getRuleIndex();
		Integer stackLevel = walker.currentStackLevel(ruleIndex);
		Map<String, Object> subMap = walker.getNodeMap(ruleIndex, stackLevel);
		subMap.remove(ASTWALKER_RULE_TYPE_KEY);
	...
	@Override
	public void exitTimestamp_literal(SQLSelectParserParser.Timestamp_literalContext ctx) {
		int ruleIndex = ctx.getRuleIndex();
		Integer stackLevel = walker.currentStackLevel(ruleIndex);
		Map<String, Object> subMap = walker.getNodeMap(ruleIndex, stackLevel);
		subMap.remove(ASTWALKER_RULE_TYPE_KEY);
