...
- **17.6.7** (Aug 2026) — triple-tuple **subquery-backed FROM** paired variants (5 tests) for all `triple*` join chains; `p_src`/`q_src`/`m1`… alias → `queryN` with operand materialization on query-backed sources; in `-Pphase15-derived-gate` and smoketest multi-modifier gate (7).
...
5. ~~**17.6.7**~~ ✅ (Aug 2026) — subquery-backed triple-tuple variants (five paired `*SubqueryFromV17_6_7Test` methods).
