...
| **17.7.10** *(optional)* | **Diagnostic (4): derived ref qualified with source alias outside primary** | ⏸️ Optional | **Not implemented.** When a column ref **outside** `table_primary` / `tuple_primary` (JOIN ON, WHERE, SELECT list, etc. — anywhere outside the relational-modifier grammar walk) names a **structured derived output** (`derivation.derived_columns`) and the qualifier matches the modifier’s **immediate source table alias** (`p_src`, `u_src`, … from `FROM … alias`) but **not** the relational-operator alias (`p`, `u`, `q`), emit **SEVERE_WARNING** (e.g. `RELATIONAL_MODIFIER_DERIVED_REFERENCE_USE_MODIFIER_ALIAS`). Message: poorly formed / confusing — derived outputs should be qualified with the **modifier alias**, not the source-primary alias. **Resolution policy:** **allow both** `p` and `p_src` (and both entries on published `table_alias`); do **not** fatal or reject the ref. Example: `ON p_src.jan_sales_SUM = …` vs `ON p.jan_sales_SUM = …` on a triple pivot/unpivot join — former warns, latter clean. Do **not** warn when the qualifier is the modifier alias, a `queryN` ref, or an unrelated join alias. Hook: dedicated visitor over archived `filters` / JOIN ON after structured `derivation` is visible (convert egress). |
| **17.7.11** | **Query-backed sources: operand tokens on `query_dictionary` (design + implement)** | ⏸️ **Design with human** | **Sign-off gate:** cannot fully sign off on **17.6.7** / the five `*SubqueryFromV17_6_7Test` methods until this step is complete and goldens match physical `triple*` operand lineage on `query_dictionary`. **Gap after 17.6.7 / 17.7.8:** Subquery- and `queryN`-backed PIVOT/UNPIVOT must mirror physical-table operand lineage — pivot/unpivot **source operand** site tokens belong on the **defining query’s** `query_dictionary` (`query0`, …), not as spurious keys in top-level `table_dictionary`, and `derivation.source_columns` stays **interface refs only** (no token blobs in goldens). **Do not** persist operand token maps on published `derivation`, helper-wide bucket stores, or convert-only side maps that leak across siblings or tuple tests. **Proposed shape (for review):** at **modifier finalize** (`finalizeRelationalModifierScope`), while walk-time `RELATIONAL_MODIFIER_SOURCE_COLUMNS` (column→tokens) is still on the **local modifier symbol table**, classify immediate source (**physical table** vs **query-backed** / non-table tuple), merge tokens into the correct dictionary once, consume matching `unresolved_column` entries by **site** (per sibling bucket), then **remove** the ephemeral token map before parent `derivation` is published. Convert egress only **publishes** interface lineage and runs existing operand resolution for **physical** join targets; it must not re-derive query-backed operand placement from flat unqualified unresolved alone. **Open for joint design:** triple-JOIN alias registration order (`p_src` / `q_src` / `r_src` vs `queryN`), UNPIVOT IN on wide subqueries, `tuple_primary` vs `table_primary`, parity with `triplePivotJoinDerivedColumnsAcrossTuplesV1Test` vs `*SubqueryFromV17_6_7Test`. **Sign-off tests (Aug 2026 baseline goldens — expect `query_dictionary` operand parity updates after implement):** five `*SubqueryFromV17_6_7Test`; ten single-modifier `*FromV17_7_11Test` covering each non-`db_object_name` `tuple_source_primary` / `table_source_primary` arm — `singlePivotSubqueryFromV17_7_11Test`, `singlePivotVariableFromV17_7_11Test`, `singlePivotJinjaFromV17_7_11Test`, `singlePivotValuesFromV17_7_11Test`, `singlePivotTableFunctionFromV17_7_11Test`, `singleUnpivotSubqueryFromV17_7_11Test`, `singleUnpivotVariableFromV17_7_11Test`, `singleUnpivotJinjaFromV17_7_11Test`, `singleUnpivotValuesFromV17_7_11Test`, `singleUnpivotTableFunctionFromV17_7_11Test` (in `-Pphase15-derived-gate` + smoketest multi-modifier gate). *Exploratory scratch on this item was rolled back (Aug 2026) — implement only after agreed design.* |
...
- [x] **E** — multi-modifier + 17.7 contract: `triple*`, `pivotUnqualifiedOuterOutputs*` (SEVERE + bundled ERROR site asserts), gate tests, **17.7.8** `closeout17_7_8_*`, **17.7.7-gap-fill** `gapFill17_7_7_*` (×11). **Signed off Aug 2026** with class at **117/117**.
...
**Status:** ✅ **Complete for Phase 17.7.7 sign-off** (Aug 2026). High-priority cells covered by `gapFill17_7_7_*` (see `phase-17.7.7-pivot-matrix-heatmap.md`). Remaining rows below are **deferred**, not blockers.
...
| **Derived** in UPDATE RHS with modifier join | Extend `pivotUpdateFromRhs*` pattern | Ambiguous derived on RHS | ⏸️ Deferred |
| **17.7.10** | `p.jan_sales_SUM` ON (no warning) | `p_src.jan_sales_SUM` ON → one SEVERE_WARNING | ⏸️ Deferred |
| **17.7.9** (optional) | — | Qualified **PIVOT-derived** name inside phrase (if grammar allows) | ⏸️ Deferred |
| **17.7.8 closeout matrix** | Physical vs subquery-backed modifier source | Unqualified ambiguous derived **vs** same-named physical column (four pairs in 17.7.8 table) | 🔄 Partial (`closeout17_7_8_*`) |

**Checklist — gap-fill (17.7.7-gap-fill):**

- [x] Matrix tag convention + **coverage heatmap** (`phase-17.7.7-pivot-matrix-heatmap.md`).
- [ ] Tag every existing `SqlEventWalkerPivotUnpivotTests` method with matrix codes (comment header or companion doc table). *(Deferred.)*
