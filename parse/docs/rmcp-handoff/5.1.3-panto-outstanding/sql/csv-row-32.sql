...
TRIM(child_names.FIRST_NAME)        as child_first_name_calc,
TRIM(child_names.LAST_NAME) as child_last_name_calc,
...
parent.high_school_grad_year_calculated as high_school_grad_year_calculated,
parent.student_funnel_stage_calc as student_funnel_stage_calc
...
Left join ( SELECT distinct Contact_key, dummy_email 
			  FROM (
					  SELECT  a.<Contact Key> Contact_key, a.<Dummy Email> dummy_email, 
							  row_number() over (partition by a.<Contact Key> order by a.CREATE_DT asc) rnum
						FROM <[Partner_Data_Platform].[Contact Acquia Crosswalk]> as a
					) as xw where xw.rnum=1 ) as xwalk 
on con.<Contact Key> = xwalk.<Contact Key>
...
left join
(
select  con3.student_contact_key,
...
from
	(
		select distinct con2.student_contact_key,
				...
				con2.student_funnel_stage_calc as student_funnel_stage_calc, rank() over (order by con2.sourcecontact_intake_dt)as rankid2
		   from
		   (
				   SELECT distinct rel.student_contact_key,scst.create_date sourcecontact_intake_dt,
						  con1.<Grade Level> as stu_hs_grade_level_calc,
						  con1.<Graduation Year> as high_school_grad_year_calculated,
						  con1.<Funnel Status Calc> as student_funnel_stage_calc, rank() over (order by int.INTAKE_PRIORITY) as rankid
...
on con.<Contact Key> = parent.student_contact_key
/*Left join 
...
-- Exclude the Students who have null source label 			   
			   ...
			   and (TRIM(label.source_label)  is not null 
