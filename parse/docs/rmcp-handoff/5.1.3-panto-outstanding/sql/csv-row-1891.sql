...
<eab_exam_score.SATR Composite Score> as eab_standard_value,
calculated_field_2 AS partner_value,
count(*) as record_count
from <[Enroll360].[Student Test Scores SIS].{fulfillment}> stud_test_scores
group by <eab_exam_score>,
...
<eab_exam_score.ACT Composite Score> as eab_standard_value,
exam_score AS partner_value,
count(*) as record_count
from <[Enroll360].[Student Test Scores SIS].{fulfillment}> stud_test_scores
group by <eab_exam_score>,
...
union all
/*
--eab_expense_group part commented out by @syeravadekar
...
COALESCE(HIGH_SCHOOL_GPA, CALCULATED_FIELD_1) as partner_value,
count(*) as record_count
from <[Enroll360].[Student High School SIS].{fulfillment}> stud_term_app
group by <eab_hs_gpa>, COALESCE(HIGH_SCHOOL_GPA, CALCULATED_FIELD_1)
...
point_record_indicator as partner_value,
count(*) as record_count
from <[Enroll360].[Student High School SIS].{fulfillment}> stud_term_app
...
--eab_fund_amount
Select
'eab_fund_amount' as field_name,
<eab_fund_amount> as eab_standard_value,
COALESCE(accept_amount, offered_amount, declined_amount, cancel_amount) as partner_value,
count(*) as record_count
from <[Enroll360].[Student Year Funds].{fulfillment}> stud_funds
group by <eab_fund_amount>, COALESCE(accept_amount, offered_amount, declined_amount, cancel_amount)

union all

--eab_fund_date
Select
'eab_fund_date' as field_name,
<eab_fund_date> as eab_standard_value,
COALESCE(accept_date, offered_date, declined_date, cancel_date) as partner_value,
count(*) as record_count
from <[Enroll360].[Student Year Funds].{fulfillment}> stud_funds
group by <eab_fund_date>, COALESCE(accept_date, offered_date, declined_date, cancel_date)