/*2025-08-08: eab_opt_out_ind logic edited to set Unmapped Value to null per ESANALYTIC-46889 - ccreekman*/
/*2025-07-07: added join extension,
added partnercontact_id, partner_system_name per ESANALYTIC-50473 - ccreekman*/
/*1-15-2025: creating updated v4 student query after auditing feed and due to updates per ESANALYTIC-44721 - ccreekman
tenant query still necessary due to 
- ec/ec2 fulfillment populations
- last and most recent campus visit fields, fields are only present in ec and have non-standard formatting that must be corrected with code, currently code for this is in bindings
*/
...
nullif(trim(regexp_replace(agg2.eab_student_id,'"|[\\000-\\037\\177]','')),'') as partnercontact_id,
nullif(trim(regexp_replace(agg2.eab_student_id_type,'"|[\\000-\\037\\177]','')),'') as partner_system_name,
agg2.crm_student_id,
...
case when event_agg.primary_student_id is not null then 'TRUE' else stud.campus_visit_indicator end as campus_visit_ind,
event_agg.min_event_date as first_campus_visit_dt,
event_agg.max_event_date as last_campus_visit_dt,

case when event_agg.primary_student_id is not null
...
from <[Enroll360].[Student].{fulfillment_entering_class}> stud
...
left outer join 
    (
    select events2.primary_student_id, count(events2.primary_student_id) as event_ct, 
...
    	from <[Enroll360].[Student Events].{final}> as se
	...
	    from <[Enroll360].[Student].{fulfillment_entering_class}> as s
	    union all
	    select s.primary_student_id, try_to_timestamp(substring(s.MOST_RECENT_CAMPUS_VISIT_DATE,1,19)) as event_date
	    from <[Enroll360].[Student].{fulfillment_entering_class}> as s
		/*union all
		select s.primary_student_id, try_to_timestamp(substring(s.first_campus_visit_date,1,19)) as event_date
	    from <[Enroll360].[Student].{fulfillment_entering_class2}> as s
	    union all
	    select s.primary_student_id, try_to_timestamp(substring(s.MOST_RECENT_CAMPUS_VISIT_DATE,1,19)) as event_date
	    from <[Enroll360].[Student].{fulfillment_entering_class2}> as s*/
    	) events 
...
    ) events2
    group by primary_student_id
) event_agg
on stud.primary_student_id = event_agg.primary_student_id
...
from <[Enroll360].[Student].{fulfillment_entering_class2}> stud
