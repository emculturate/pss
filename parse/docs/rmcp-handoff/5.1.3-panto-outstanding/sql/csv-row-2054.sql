/*2025-08-04: created blank fulfillment queries to get back on master convert and final queries with out complicating bindings set up - ccreekman*/
/*11-04-2024: switched STA to single file master, PCM references {fulfillment} instead of {fulfillment_ug_applicants} - ccreekman*/
/*06-03-2024: Updated to PCM Standard for Term, Type, and Year starting on line 427. wjl*/
select distinct 
agg.field_name,
agg.eab_standard_value,
agg.partner_value,
agg.record_count
...
/**************************************
Result set from Enroll360.Partner Code Mapping.convert_ug_inquiries
***************************************/  

select field_name, eab_standard_value, partner_value, record_count
from <[Enroll360].[Partner Code Mapping].{convert}> pcm
...
count(*) as record_count
from <[Enroll360].[Student Phone].{fulfillment_ug_applicants}> stud_phone
...
count(*) as record_count
from <[Enroll360].[Student Email].{fulfillment_ug_applicants}> stud_email
...
count(*) as record_count
from <[Enroll360].[Student Test Scores].{fulfillment_ug_applicants}> stud_test_scores
...
count(*) as record_count
from <[Enroll360].[Student Relationship].{fulfillment_ug_applicants}> stud_rship
...
count(*) as record_count
from <[Enroll360].[Student Term].{fulfillment_ug_applicants}> stud_term
...
-- Updated to PCM Standard for Term, Type, and Year.  
/*****************
Current Entry Term
*****************/
select 
...
country,
count(*) as record_count
from <[Enroll360].[Student Address].{fulfillment_ug_applicants}> stud_addr
...
count(*) as record_count
from <[Enroll360].[Student Address].{fulfillment_ug_applicants}> stud_addr
where calculated_field_7 is not null
group by calculated_field_7*/
) countries
group by country
...
count(*) as record_count
from <[Enroll360].[Student Race].{fulfillment_ug_applicants}> stud
group by <eab_race>,
race
...
--Application Started Indicator /*only in ug_inqs*/
select 
...
--Application Completed Indicator /*only in ug_inqs*/
select 
