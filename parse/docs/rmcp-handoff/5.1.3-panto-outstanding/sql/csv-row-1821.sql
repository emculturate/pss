/*2025-11-18: copied from new Master and brought in customizations:
--Enrollment Indicator for 2024 is from supplemental table, 2025 data uses regular enrollment_indicator field - ehatfield, ccreekman*/
/*2025-11-17: new fulfillment attributes created, conversions have been updated to reference the new non-"eab_" field:
point_record_indicator, high_school_gpa_type, expense_group_key
added conversions for single parent flag/indicator (not a standardized field sent downstream but 1:1 conversions used for logic)
student type added to entities, code updated to reflect all entities with student_type values - ccreekman*/
...
where expense_group_key is not null
...
--Student Year Finaid Application
select 
syfa.student_type as partner_value,
count(*) as record_count
from <[Enroll360].[Student Year Finaid Application].{fulfillment}> syfa
group by syfa.student_type

union all

--Student Year Funds
select 
syf.student_type as partner_value,
count(*) as record_count
from <[Enroll360].[Student Year Funds].{fulfillment}> syf
group by syf.student_type
...
--Enrollment Indicator - from 2024 update table
select 
'eab_enroll_ind' as field_name,
<eab_enroll_ind> as eab_standard_value,
enrollment_indicator as partner_value,
count(*) as record_count
from <[Enroll360].[Student Term Application SIS].{fulfillment_enrolls}> stud_term_app
...
high_school_gpa_type as partner_value, --non-standard field not available 
count(*) as record_count
from <[Enroll360].[Student High School SIS].{fulfillment}> stud_hs
group by <eab_hs_gpa_type>,
high_school_gpa_type
...
point_record_indicator as partner_value, --non-standard field not available 
count(*) as record_count
from <[Enroll360].[Student High School SIS].{fulfillment}> stud_hs
group by <eab_point_record_ind>,
point_record_indicator
