...
SELECT
    ...
    null as CAMPUS_VISIT_COUNT,
    null as CAMPUS_VISIT_INDICATOR,
    ...
    null as FIRST_CAMPUS_VISIT_DATE,
    ...
    null as MOST_RECENT_CAMPUS_VISIT_DATE,
    CASE
		WHEN NULLIF(TRIM(TO_VARCHAR(p.status)), '') = 'Current Student' THEN '1'
        WHEN COALESCE(NULLIF(TRIM(TO_VARCHAR(la.ok_to_email)), ''), NULLIF(TRIM(TO_VARCHAR(p.ok_to_email)), '')) IS NULL THEN NULL
        WHEN CAST(COALESCE(NULLIF(TRIM(TO_VARCHAR(la.ok_to_email)), ''), NULLIF(TRIM(TO_VARCHAR(p.ok_to_email)), '')) AS VARCHAR) = '0' then '1'
	...
	null as ACADEMIC_COHORT, 
	null as ACADEMIC_SCHOOL_OF_STUDY, 
	null as CLIENT_FILE_NAME, 
	null as EMPLOYER, 
	SUBSTRING(CURRENT_TIMESTAMP(), 1, 19) as INTAKE_DT, 
	null as ORIGIN_SOURCE_CODE_EAB, 
	null as PREFIX, 
	p.suffix as SUFFIX, 
	CASE
		WHEN NULLIF(TRIM(TO_VARCHAR(p.status)), '') = 'Current Student' THEN '1'
