select distinct
agg2.primary_student_id,
agg2.sis_student_id,
agg2.eab_student_id,
agg2.crm_student_id,
agg2.other_student_id,
trim(regexp_replace(agg2.fname,'[^[:ascii:]]|[\\0]','')) as fname,
trim(regexp_replace(agg2.mname,'[^[:ascii:]]|[\\0]','')) as mname,
trim(regexp_replace(agg2.lname,'[^[:ascii:]]|[\\0]','')) as lname,
trim(regexp_replace(agg2.preferred_fname,'[^[:ascii:]]|[\\0]','')) as preferred_fname,
...
case 
	when spring_summer_2023_admits.st_primary_student_id is not null then 'true' 
	else agg2.eab_opt_out_ind 
...
try_to_timestamp(replace(stud.birth_date,'"','')) as birth_dt,
stud.gender,
...
try_to_timestamp(stud.origin_source_date) as origin_source_dt,
...
stud.ethnicity,
case when stud_event.record_count is not null then cast(stud_event.record_count as varchar) else stud.campus_visit_count end as campus_visit_cnt,
case when stud_event.primary_student_id is not null then 'true' else stud.campus_visit_indicator end as campus_visit_ind,
case when stud_event.min_date is not null then stud_event.min_date else try_to_timestamp(stud.first_campus_visit_date) end as first_campus_visit_dt,
case when stud_event.max_date is not null then stud_event.max_date else try_to_timestamp(stud.most_recent_campus_visit_date) end as last_campus_visit_dt,

case when stud_event.primary_student_id is not null
...
case 
 when non_freshmen.primary_student_id is not null then 'true'
 when pcm_int_ind.eab_standard_value = 'true' then 'true'
 else pcm_optout_ind.eab_standard_value 
 end as eab_opt_out_ind,
...
left outer join (
  ...
  from <[Enroll360].[Student Term].{final}> as st
  where 1=1
  and eab_student_type <> 'Freshman'
   ) non_freshmen
  on stud.primary_student_id = non_freshmen.primary_student_id
...
left outer join (
    select stud_event.primary_student_id, min(stud_event.event_dt) as min_date, 
    max(stud_event.event_dt) as max_date, count(distinct stud_event.event_dt) as record_count
    from <[Enroll360].[Student Events].{final}> stud_event
    where stud_event.event_dt is not null
    group by stud_event.primary_student_id
    order by stud_event.primary_student_id
) stud_event
on stud.primary_student_id = stud_event.primary_student_id 
...
left outer join (
  select st.primary_student_id as st_primary_student_id
  from <[Enroll360].[Student Term].{final}> as st
  where 1=1
  and eab_entry_year_academic = '2023'
  and eab_entry_term in ('Spring','Summer')
  and eab_current_funnel_status = 'Uncommitted Admit'
) spring_summer_2023_admits
   on agg2.primary_student_id = spring_summer_2023_admits.st_primary_student_id		  
