...
PARSE = ROOT / "parse"
CAPTURE_PROPS = PARSE / "target/dml_substitution_capture.properties"
...
def method_name_for_case(prefix: str, suffix: str) -> str:
    if prefix == "insert":
        return f"insertComplexSubstitution{suffix}"
    if prefix == "update":
        return f"updateComplexSubstitution{suffix}"
    return f"deleteComplexSubstitution{suffix}"


def write_capture_properties(cases: list[tuple[str, str, list[str]]]) -> None:
    CAPTURE_PROPS.parent.mkdir(parents=True, exist_ok=True)
    lines: list[str] = []
    for prefix, suffix, sql_lines in cases:
        method = method_name_for_case(prefix, suffix)
        sql = "\n".join(sql_lines)
        escaped = sql.replace("\\", "\\\\").replace("\n", "\\n").replace("\r", "")
        lines.append(f"{method}={escaped}\n")
    CAPTURE_PROPS.write_text("".join(lines), encoding="utf-8")


def run_properties_capture() -> dict[str, dict[str, str]]:
    subprocess.run(["mvn", "-q", "test-compile", "-DskipTests"], cwd=PARSE, check=True)
    ...
    full_cp = f"{PARSE / 'target/test-classes'}:{PARSE / 'target/classes'}:{cp}"
    out = subprocess.run(
        ["java", "-cp", full_cp, "sql.walker.WalkerGoldenCaptureOnce", str(CAPTURE_PROPS)],
        cwd=PARSE,
        text=True,
        capture_output=True,
    )
    combined = out.stdout + out.stderr
    (PARSE / "target/dml_probe_output.txt").write_text(combined)
    ...
    for line in combined.splitlines():
        ...
        goldens.setdefault(method, {})[field] = value.replace("\\n", "\n")
    return goldens
...
def main() -> None:
    ...
    write_capture_properties(all_cases)
    print(f"Wrote capture properties with {len(all_cases)} cases")
    goldens = run_properties_capture()
